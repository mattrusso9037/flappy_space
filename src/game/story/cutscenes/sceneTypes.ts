...
export type SceneActorKind = 'ship' | 'pilot' | 'wormhole' | 'repair-sparks' | 'tether';
...
export function validateScene(scene: CinematicScene, duration: number): string[] {
  ...
  for (const actor of scene.actors) {
    ...
    ids.add(actor.id);
    if (!['ship', 'pilot', 'wormhole', 'repair-sparks', 'tether'].includes(actor.kind)) errors.push('Unknown actor kind.');
